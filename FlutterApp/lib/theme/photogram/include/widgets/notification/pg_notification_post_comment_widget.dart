...
class PgNotificationPostCommentWidget extends StatefulWidget {
  final int notificationId;

  const PgNotificationPostCommentWidget({
    Key? key,
    required this.notificationId,
  }) : super(key: key);

  @override
  State<PgNotificationPostCommentWidget> createState() => _PgNotificationPostCommentWidgetState();
}

class _PgNotificationPostCommentWidgetState extends State<PgNotificationPostCommentWidget>
  ...
  void onLoadEvent() {
    ...
    _onPhotoOfYou = _notificationModel.metaType == NotificationEnum.metaTypePostCommentOnPhotoOfYou;
  }
  ...
  Widget _buildTitle() {
    ...
    textSpans.add(
      TextSpan(
        text: _notificationModel.linkedContent.collections[UserTable.tableName]!.length > 2
            ? sprintf(
                _onPhotoOfYou
                    ? AppLocalizations.of(context)!.commentedOnPhotoOfYou
                    : AppLocalizations.of(context)!.andNOthersCommentedOnYourPost,
                [
                  _notificationModel.linkedContent.collections[UserTable.tableName]!.length - _userModels.length,
                ],
              )
            : _onPhotoOfYou
                ? AppLocalizations.of(context)!.commentedOnPhotoOfYou
                : AppLocalizations.of(context)!.commentedOnYourPost,
  ...
  Widget _buildTrailing() {
    return GestureDetector(
      onTap: () => PgUtils.openPostcommentsPage(
        context: context,
        postId: _targetPostModel.intId,
        focus: false,
